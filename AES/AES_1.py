# AES S-box
SBOX = [
    
]

# AES Rijndael round constants
RCON = (
    0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36, 0x6C, 0xD8, 0xAB, 0x4D, 0x9A
)

# Key expansion helper functions
def key_schedule_core(word, i):
    # Rotate word
    word = ((word << 8) & 0xFFFFFFFF) | (word >> 24)
    # Apply S-box to each byte
    word = (SBOX[(word >> 24) & 0xFF] << 24) | (SBOX[(word >> 16) & 0xFF] << 16) | \
           (SBOX[(word >> 8) & 0xFF] << 8) | SBOX[word & 0xFF]
    # XOR with RCON
    word ^= (RCON[i] << 24)
    return word

def key_expansion(key):
    key_words = [int(key[i:i+8], 16) for i in range(0, 32, 8)]
    expanded_key = key_words.copy()
    i = 4  # Start with the first word beyond the original key
    while i < 44:
        temp = expanded_key[i-1]
        if i % 4 == 0:
            temp = key_schedule_core(temp, i // 4)
        expanded_key.append(expanded_key[i-4] ^ temp)
        i += 1
    return expanded_key

# SubBytes transformation
def sub_bytes(state):
    return [[SBOX[state[i][j]] for j in range(4)] for i in range(4)]

def transpose(matrix):
    return [[matrix[j][i] for j in range(4)] for i in range(4)]
def shift_rows(state):
    temp =  [
        [transpose(state)[i][(j + i) % 4] for j in range(4)]
        for i in range(4)
    ]
    return(transpose(temp))


# MixColumns transformation
def mix_columns_(state):
    def multiply(a, b):
        p = 0
        for _ in range(8):
            if b & 1:
                p ^= a
            hi_bit_set = a & 0x80
            a <<= 1
            if hi_bit_set:
                a ^= 0x1B  # AES irreducible polynomial
            b >>= 1
        return p

    result = [[0] * 4 for _ in range(4)]
    for i in range(4):
        for j in range(4):
            result[i][j] = (
                multiply(0x02, state[i][j]) ^
                multiply(0x03, state[(i + 1) % 4][j]) ^
                state[(i + 2) % 4][j] ^
                state[(i + 3) % 4][j]
            )
    return result



def xtime(a: int) -> int:
    if a & 0x80:
        return ((a << 1) ^ 0x1b) & 0xff
    return a << 1


def mix_column(col: [int]):
    c_0 = col[0]
    all_xor = col[0] ^ col[1] ^ col[2] ^ col[3]
    col[0] ^= all_xor ^ xtime(col[0] ^ col[1])
    col[1] ^= all_xor ^ xtime(col[1] ^ col[2])
    col[2] ^= all_xor ^ xtime(col[2] ^ col[3])
    col[3] ^= all_xor ^ xtime(c_0 ^ col[3])


def mix_columns(state: [[int]]):
    for r in state:
        mix_column(r)

# AddRoundKey transformation
def add_round_key(state, round_key):
    return [[state[i][j] ^ ((round_key[i] >> (24 - 8 * j)) & 0xFF) for j in range(4)] for i in range(4)]

# AES encryption function
def aes_encrypt(plaintext, key):
    state = [[int(plaintext[i:i+2], 16) for i in range(j, j + 8, 2)] for j in range(0, 32, 8)]
    print(state)
    expanded_key = key_expansion(key)

    # Initial round key addition
    state = add_round_key(state, expanded_key[:4])
    print("Round",state)

    # Main rounds
    for round_num in range(1, 10):
        state = sub_bytes(state)
        print(f"state_sb: {state}")
        state = shift_rows(state)
        print(f"state_sr: {state}")
        state = mix_columns(state)
        print(f"state_mix: {state}")
        state = add_round_key(state, expanded_key[4 * round_num:4 * (round_num + 1)])
        print(f"state_add: {state}")
        print(round_num)

    # Final round
    state = sub_bytes(state)
    state = shift_rows(state)
    state = add_round_key(state, expanded_key[40:])

    # Convert state to a flat list of hex strings
    result = [format(state[i][j], '02x') for i in range(4) for j in range(4)]
    return ''.join(result)



# Example usage
plaintext = "54776f204f6e65204e696e652054776f"
key = "5468617473206d79204b756e67204675"

ciphertext = aes_encrypt(plaintext, key)
print("Ciphertext:", ciphertext)

