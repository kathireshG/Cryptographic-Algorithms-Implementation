...
def encrypt(pt, rkb, rk):
    ...
    right = pt[32:64]
    for i in range(16):
        right_expanded = permute(right, [32, 1, 2, 3, 4, 5, 4, 5, 6, 7, 8, 9, 8, 9, 10, 11,
        ...
        sbox_str = permute(sbox_str, [16, 7, 20, 21, 29, 12, 28, 17, 1, 15, 23, 26,
        ...
        combine = left + right
        cipher_text = permute(combine, [40, 8, 48, 16, 56, 24, 64, 32,
        ...
        print(f"Round {i + 1:2d}   {bin2hex(left)}   {bin2hex(right)}   {bin2hex(cipher_text)}")
...
key = hex2bin(key)
key = permute(key, [57, 49, 41, 33, 25, 17, 9, 1, 58, 50, 42, 34, 26, 18, 10, 2,
...
decrypted_message = bin2hex(encrypt(encrypted_message, rkb[::-1], rk))
